...
if resume_epoch != 0:
    if not os.path.exists(save_dir_root, 'run', 'run_*'):
        os.mkdir(save_dir_root, 'run', 'run_*')
...
modelName = 'P3D24' # Options: C3D or R2Plus1D
...
def train_model(dataset=dataset, save_dir=save_dir, num_classes=num_classes, lr=lr,
    ...
    train_dataloader = DataLoader(VideoDataset(dataset=dataset, split='train',clip_len=16), batch_size=BatchSize, shuffle=True, num_workers=4)
    val_dataloader   = DataLoader(VideoDataset(dataset=dataset, split='val',  clip_len=16), batch_size=BatchSize, num_workers=4)
    test_dataloader  = DataLoader(VideoDataset(dataset=dataset, split='test', clip_len=16), batch_size=BatchSize, num_workers=4)
