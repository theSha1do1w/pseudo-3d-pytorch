...
class VideoDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return torch.from_numpy(buffer), torch.from_numpy(labels), self.filenames[index]
    ...
    def check_preprocess(self):
        ...
        for ii, video_class in enumerate(os.listdir(os.path.join(self.output_dir, 'train'))):
            for video in os.listdir(os.path.join(self.output_dir, 'train', video_class)):
                ...
                if np.shape(image)[0] != 160 or np.shape(image)[1] != 160:
                    return False
                else:
                    break
    ...
    def preprocess(self):
        if not os.path.exists(self.output_dir):
            ...
            #os.mkdir(self.output_dir, 'train')
            #os.mkdir(self.output_dir, 'val')
            os.mkdir(self.output_dir, 'test')
    ...
    def crop(self, buffer, clip_len, crop_size):
        ...
        if buffer.shape[2] == crop_size and buffer.shape[3] == crop_size:
...
if __name__ == "__main__":
    ...
    train_data = VideoDataset(dataset='future1', split='test', clip_len=1)
